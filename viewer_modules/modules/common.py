...
class ViewerInterface:
    ...
    def VIEWER_discard_workEnv(self, clear_sample_id=False):
        if self.viewer_ref.workEnv is None:
            return True
        if (self.viewer_ref.workEnv.saved is False) and (QtWidgets.QMessageBox.warning(self.viewer_ref, 'Warning!',
            ...
            return False
        self.viewer_ref.clearWorkEnv(clear_sample_id)
        return True

    def VIEWER_clear_workEnv(self, clear_sample_id=False):
...
class BatchRunInterface(metaclass=abc.ABCMeta):
    @staticmethod
    @abc.abstractmethod
