...
class ImageMenu:
    ...
    def crop(self):
        if self.crop_roi is None:
            self.vi.viewer_ref.status_bar.showMessage('Create your crop region and then click on "Crop" again in the '
            ...
            self.vi.viewer_ref.status_bar.showMessage('Cropping to your selection, please wait')
            ...
            self.vi.VIEWER_update_workEnv()
            self.vi.viewer_ref.status_bar.showMessage('Cropping completed!')
    ...
    def draw_measure_line(self, ev):
        if self.measure_line_ is None:
            ...
            self.vi.viewer_ref.status_bar.showMessage('Click on a second point in the image to '
    ...
    def measure_tool(self, ev=False):
        if self.measure_line is None:
            self.vi.viewer_ref.status_bar.showMessage('Click on a point in the image to draw the first point of a line')
